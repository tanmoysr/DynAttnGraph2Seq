...
class GatedMeanAggregator(Layer):
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
        ...
        if concat:
            ...
            # self.output_dim = output_dim # edited by TC
...
class MeanAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        # [nodes] x [out_dim]
        from_neighs = tf.matmul(neigh_means, self.vars['neigh_weights'])
        # from_neighs = tf.tensordot(neigh_means, self.vars['neigh_weights'], [[2], [0]]) # Edited by TC

        from_self = tf.matmul(self_vecs, self.vars["self_weights"])
        # from_self = tf.tensordot(self_vecs, self.vars['self_weights'], [[2], [0]])  # Edited by TC
