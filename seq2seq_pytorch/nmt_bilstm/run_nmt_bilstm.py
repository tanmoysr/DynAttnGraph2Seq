...
import data_processor_bilstm as data_processor
import model_bilstm as model_s2s
...
def main(run_mode):
    print('Running for {} forum.'.format(config.forum_name))
    ...
    # define model
    encoder = model_s2s.Encoder(vocab_inp_size, embedding_dim, units, BATCH_SIZE, dropout, layer, config.post_encodder).to(device)
